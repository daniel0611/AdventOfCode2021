...
impl Cube {
    fn parse(s: &str) -> Cube {
        let (on_str, coordinates_str) = s.split_once(' ').unwrap();
        let on = on_str == "on";

        let coordinates = coordinates_str.split(',');
        let coo = coordinates
            .map(|coordinate| {
                let (_axis, range) = coordinate.split_once('=').unwrap();
...
mod tests {
    ...
    const TEST_INPUT: &str = "on x=-5..47,y=-31..22,z=-19..33\n\
